...
data = pd.read_csv("./data/feature_data/S125_fea_table_both.csv")  # 读取数据
...
pred_ddg_both.to_excel("./evaluation/pred_ddg_both.xlsx", index=True)
...
evaluate_result = pd.DataFrame(
    ...
    columns=['prediction_on_S125_both'],
)

evaluate_result.to_excel("./evaluation/evaluate_both.xlsx", index=True)  # 保存评估结果
